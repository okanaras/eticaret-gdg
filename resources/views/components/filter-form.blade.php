            ...
            <div class="col mb-3">
                ...
                @if ($filter['type'] == 'text')
                    <input type="text" class="form-control" id="{{ $filter['column'] }}" autocomplete="off"
                        placeholder="{{ $filter['label'] }}" name="{{ $filter['column'] }}"
                        value="{{ request($filter['column']) }}">
                @elseif($filter['type'] == 'number')
                    <input type="number" class="form-control" id="{{ $filter['column'] }}" autocomplete="off"
                        placeholder="{{ $filter['label'] }}" name="{{ $filter['column'] }}"
                        value="{{ request($filter['column']) }}">
                ...
                @elseif($filter['type'] == 'date')
                    <div class="input-group flatpickr" id="flatpickr-date">
                        <input type="text" class="form-control" placeholder="{{ $filter['label'] }}" data-input
                            name="{{ $filter['column'] }}" value="{{ request($filter['column']) }}">
